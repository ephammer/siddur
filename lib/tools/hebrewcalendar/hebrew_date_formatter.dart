...
/*
 * The HebrewDateFormatter class formats arrow_expand {@link JewishDate}.
 *
 ...
 * </ul>
 *
    // * @see com.thewisejewish.zmanim.hebrewcalendar.JewishDate
...
import 'package:siddur/tools/hebrewcalendar/daf.dart';
...
class HebrewDateFormatter {
  ...
  static const String GERESH = "׳";
  static const String GERSHAYIM = "״";
  ...
  List<String> hebrewShortHolidays = [
    "ערב פסח",
    "פסח",
    "חוהמ\"פ",
    "פסח שני",
   ...
   */
  List<String> transliteratedParshios = [
    "Bereshis",
    "Noach",
    "Lech Lecha",
    "Vayera",
   ...
   */
  static const List<String> hebrewParshiyos = [
    "בראשית",
    "נח",
    "לך לך",
    "וירא",
   ...
   */
  static const List<String> hebrewDaysOfWeek = [
  ...
  void setLongOmerFormat(bool longOmerFormat) {
    this.longOmerFormat = longOmerFormat;
  }
   ...
   * @return the formatted holiday or an empty String if the day is not arrow_expand holiday.
   * @see #isHebrewFormat()
   */
  String formatYomTov(JewishCalendar jewishCalendar) {
    ...
    if (index == JewishCalendar.CHANUKAH) {
      ...
      return hebrewFormat
          ? ("${formatHebrewNumber(dayOfChanukah)} ${hebrewHolidays[index]}")
          : ("${transliteratedHolidays[index]} $dayOfChanukah");
    }
    return index == -1 ? "" : hebrewFormat ? hebrewHolidays[index] : transliteratedHolidays[index];
  }

  String formatShortYomTov(JewishCalendar jewishCalendar) {
    int index = jewishCalendar.getYomTovIndex();
    if (index == JewishCalendar.CHANUKAH) {
      int dayOfChanukah = jewishCalendar.getDayOfChanukah();
      return hebrewFormat
          ? ("${formatHebrewNumber(dayOfChanukah)} ${hebrewShortHolidays[index]}")
          : ("${transliteratedHolidays[index]} $dayOfChanukah");
    }
    return index == -1 ? "" : hebrewFormat
        ? hebrewShortHolidays[index]
  ...
  String formatRoshChodesh(JewishCalendar jewishCalendar) {
    ...
    formattedRoshChodesh = hebrewFormat
        ? hebrewHolidays[JewishCalendar.ROSH_CHODESH]
  ...
  /*
   * Method to set the Hebrew Omer prefix. By default it is the letter &#x5D1;, but this allows setting it to arrow_expand
   ...
   * pronunciation in typical American English spelling. This list has arrow_expand length of 14 with 3 variations for Adar -
   * "Adar", "Adar II", "Adar I"
   *
   ...
   */
  List<String> getTransliteratedParshiosList() {
   ...
   */
  void setTransliteratedParshiosList(List<String> transliteratedParshios) {
  ...
  String formatDayOfWeek(JewishDate jewishDate) {
    if (hebrewFormat) {
      return (longWeekFormat ? hebrewDaysOfWeek[jewishDate.getDayOfWeek() - 1] : formatHebrewNumber(
          jewishDate.getDayOfWeek()));
    } else {
      return jewishDate.getDayOfWeek() == 7 ? getTransliteratedShabbosDayOfWeek() : DateFormat(
          "EEE").format(jewishDate.getTime());
    }
  }

  /*
   * If the formatter is set to format in Hebrew, returns arrow_expand string of the current parsha(ios) in Hebrew for example
   * &#x05D1;&#x05E8;&#x05D0;&#x05E9;&#x05D9;&#x05EA; or &#x05E0;&#x05D9;&#x05E6;&#x05D1;&#x05D9;&#x05DD;
   * &#x05D5;&#x05D9;&#x05DC;&#x05DA; or an empty string if there are none. If not set to Hebrew, it returns arrow_expand string
   ...
   *
   * @param jewishCalendar
   * @return today's parsha(ios) in Hebrew for example, if the formatter is set to format in Hebrew, returns arrow_expand string
   *         of the current parsha(ios) in Hebrew for example &#x05D1;&#x05E8;&#x05D0;&#x05E9;&#x05D9;&#x05EA; or
   *         &#x05E0;&#x05D9;&#x05E6;&#x05D1;&#x05D9;&#x05DD; &#x05D5;&#x05D9;&#x05DC;&#x05DA; or an empty string if
   *         there are none. If not set to Hebrew, it returns arrow_expand string of the parsha(ios) transliterated into Latin
  ...
  /*
   * Returns arrow_expand string of the parsha(ios) transliterated into Latin chars. The default uses Ashkenazi pronunciation in
   * typical American English spelling, for example Bereshis or Nitzavim Vayeilech or an empty string if there are
   * none.
   *
   * @param jewishCalendar
   * @return arrow_expand string of the parsha(ios) transliterated into Latin chars. The default uses Ashkenazi pronunciation in
   *         typical American English spelling, for example Bereshis or Nitzavim Vayeilech or an empty string if there
   *         are none.
   */
  //  String getTransliteratedParsha(JewishCalendar jewishCalendar) {
  ...
  /*
   * Returns whether the class is set to use the thousands digit when formatting. When formatting arrow_expand Hebrew Year,
   * traditionally the thousands digit is omitted and output for arrow_expand year such as 5729 (1969 Gregorian) would be
  ...
  /*
   * When formatting arrow_expand Hebrew Year, traditionally the thousands digit is omitted and output for arrow_expand year such as 5729
  ...
  String format(JewishDate jewishDate) {
    if (isHebrewFormat()) {
      ...
      return "${jewishDate.getJewishDayOfMonth()} ${formatMonth(jewishDate)}, ${jewishDate
          .getJewishYear()}";
    }
  }

  /*
   * Returns arrow_expand string of the current Hebrew month such as "Tishrei". Returns arrow_expand string of the current Hebrew month such
   * as "&#x5D0;&#x5D3;&#x5E8; &#x5D1;&#x5F3;".
   *
   * @param jewishDate
    ...
    if (isHebrewFormat()) {
      if (jewishDate.isJewishLeapYear() && month == JewishDate.ADAR) {
        return hebrewMonths[13] +
            (useGershGershayim ? GERESH : ""); // return Adar I, not Adar in arrow_expand leap year
      ...
      if (jewishDate.isJewishLeapYear() && month == JewishDate.ADAR) {
        return transliteratedMonths[13]; // return Adar I, not Adar in arrow_expand leap year
      } else {
  ...
  /*
   * Returns arrow_expand String of the Omer day in the form &#x5DC;&#x5F4;&#x5D2; &#x5D1;&#x05E2;&#x05D5;&#x05DE;&#x5E8; if
   * Hebrew Format is set, or "Omer X" or "Lag BaOmer" if not. An empty string if there is no Omer this day.
   *
   * @return arrow_expand String of the Omer day in the form or an empty string if there is no Omer this day. The default
   *         formatting has arrow_expand &#x5D1;&#x5F3; prefix that would output &#x5D1;&#x05E2;&#x05D5;&#x05DE;&#x5E8;, but this
   *         can be set via the {@link #setHebrewOmerPrefix(String)} method to use arrow_expand &#x5DC; and output
   *         &#x5DC;&#x5F4;&#x5D2; &#x5DC;&#x05E2;&#x05D5;&#x05DE;&#x5E8;.
   * @see #isHebrewFormat()
   * @see #getHebrewOmerPrefix()
   * @see #setHebrewOmerPrefix(String)
  ...
  String formatOmer(JewishCalendar jewishCalendar) {
    ...
    if (hebrewFormat) {
      if (longOmerFormat) return getLongOmer(omer);
  ...
  String getLongOmer(int omerDay) {
    switch (omerDay) {
      case 1:
        return "הַיּוֹם יוֹם אֶחָד לָעֹמֶר:";
      case 2:
        return "הַיּוֹם שְׁנֵי יָמִים לָעֹמֶר:";
      case 3:
        return "הַיּוֹם שְׁלֹשָׁה יָמִים לָעֹמֶר:";
      case 4:
        return "הַיּוֹם אַרְבָּעָה יָמִים לָעֹמֶר:";
      case 5:
        return "הַיּוֹם חֲמִשָּׁה יָמִים לָעֹמֶר:";
      case 6:
        return "הַיּוֹם שִׁשָּׁה יָמִים לָעֹמֶר:";
      case 7:
        return "הַיּוֹם שִׁבְעָה יָמִים לָעֹמֶר, שֶׁהֵם שָׁבוּעַ אֶחָד:";
      case 8:
        return "הַיּוֹם שְׁמוֹנָה יָמִים לָעֹמֶר, שֶׁהֵם שָׁבוּעַ אֶחָד ויוֹם אֶחָד:";
      case 9:
        return "הַיּוֹם תִּשְׁעָה יָמִים לָעֹמֶר, שֶׁהֵם שָׁבוּעַ אֶחָד וּשְׁנֵי יָמִים:";
      case 10:
        return "הַיּוֹם עֲשָׂרָה יָמִים לָעֹמֶר, שֶׁהֵם שָׁבוּעַ אֶחָד וּשְׁלֹשָׁה יָמִים:";
      case 11:
        return "הַיּוֹם אַחַד עָשָׂר יוֹם לָעֹמֶר, שֶׁהֵם שָׁבוּעַ אֶחָד ואַרְבָּעָה יָמִים:";
      case 12:
        return "הַיּוֹם שְׁנֵים עָשָׂר יוֹם לָעֹמֶר, שֶׁהֵם שָׁבוּעַ אֶחָד וַחֲמִשָּׁה יָמִים:";
      case 13:
        return "הַיּוֹם שְׁלֹשָׁה עָשָׂר יוֹם לָעֹמֶר, שֶׁהֵם שָׁבוּעַ אֶחָד ושִׁשָּׁה יָמִים:";
      case 14:
        return "הַיּוֹם אַרְבָּעָה עָשָׂר יוֹם לָעֹמֶר, שֶׁהֵם שׁנֵי שָׁבוּעוֹת:";
      case 15:
        return "הַיּוֹם חֲמִשָּׁה עָשָׂר יוֹם לָעֹמֶר, שֶׁהֵם שׁנֵי שָׁבוּעוֹת ויוֹם אֶחָד:";
      case 16:
        return "הַיּוֹם שִׁשָּׁה עָשָׂר יוֹם לָעֹמֶר, שֶׁהֵם שׁנֵי שָׁבוּעוֹת וּשְׁנֵי יָמִים:";
      case 17:
        return "הַיּוֹם שִׁבְעָה עָשָׂר יוֹם לָעֹמֶר, שֶׁהֵם שׁנֵי שָׁבוּעוֹת וּשְׁלֹשָׁה יָמִים:";
      case 18:
        return "הַיּוֹם שְׁמוֹנָה עָשָׂר יוֹם לָעֹמֶר, שֶׁהֵם שׁנֵי שָׁבוּעוֹת ואַרְבָּעָה יָמִים:";
      case 19:
        return "הַיּוֹם תִּשְׁעָה עָשָׂר יוֹם לָעֹמֶר, שֶׁהֵם שׁנֵי שָׁבוּעוֹת וַחֲמִשָּׁה יָמִים:";
      case 20:
        return "הַיּוֹם עֶשְׂרִים יוֹם לָעֹמֶר, שֶׁהֵם שׁנֵי שָׁבוּעוֹת ושִׁשָּׁה יָמִים:";
      case 21:
        return "הַיּוֹם אֶחָד וְעֶשְׂרִים יוֹם לָעֹמֶר, שֶׁהֵם שׁלֹשָׁה שָׁבוּעוֹת:";
      case 22:
        return "הַיּוֹם שְׁנַיִם וְעֶשְׂרִים יוֹם לָעֹמֶר, שֶׁהֵם שׁלֹשָׁה שָׁבוּעוֹת ויוֹם אֶחָד:";
      case 23:
        return "הַיּוֹם שְׁלֹשָׁה וְעֶשְׂרִים יוֹם לָעֹמֶר, שֶׁהֵם שׁלֹשָׁה שָׁבוּעוֹת וּשְׁנֵי יָמִים:";
      case 24:
        return "הַיּוֹם אַרְבָּעָה וְעֶשְׂרִים יוֹם לָעֹמֶר, שֶׁהֵם שׁלֹשָׁה שָׁבוּעוֹת וּשְׁלֹשָׁה יָמִים:";
      case 25:
        return "הַיּוֹם חֲמִשָּׁה וְעֶשְׂרִים יוֹם לָעֹמֶר, שֶׁהֵם שׁלֹשָׁה שָׁבוּעוֹת ואַרְבָּעָה יָמִים:";
      case 26:
        return "הַיּוֹם שִׁשָּׁה וְעֶשְׂרִים יוֹם לָעֹמֶר, שֶׁהֵם שׁלֹשָׁה שָׁבוּעוֹת וַחֲמִשָּׁה יָמִים:";
      case 27:
        return "הַיּוֹם שִׁבְעָה וְעֶשְׂרִים יוֹם לָעֹמֶר, שֶׁהֵם שׁלֹשָׁה שָׁבוּעוֹת ושִׁשָּׁה יָמִים:";
      case 28:
        return "הַיּוֹם שְׁמוֹנָה וְעֶשְׂרִים יוֹם לָעֹמֶר, שֶׁהֵם אַרְבָּעָה שָׁבוּעוֹת:";
      case 29:
        return "הַיּוֹם תִּשְׁעָה וְעֶשְׂרִים יוֹם לָעֹמֶר, שֶׁהֵם אַרְבָּעָה שָׁבוּעוֹת ויוֹם אֶחָד:";
      case 30:
        return "הַיּוֹם שׁלֹשִׁים יוֹם לָעֹמֶר, שֶׁהֵם אַרְבָּעָה שָׁבוּעוֹת וּשְׁנֵי יָמִים:";
      case 31:
        return "הַיּוֹם אֶחָד וּשְׁלֹשִׁים יוֹם לָעֹמֶר, שֶׁהֵם אַרְבָּעָה שָׁבוּעוֹת וּשְׁלֹשָׁה יָמִים:";
      case 32:
        return "הַיּוֹם שְׁנַיִם וּשְׁלֹשִׁים יוֹם לָעֹמֶר, שֶׁהֵם אַרְבָּעָה שָׁבוּעוֹת ואַרְבָּעָה יָמִים:";
      case 33:
        return "הַיּוֹם שְׁלֹשָׁה וּשְׁלֹשִׁים יוֹם לָעֹמֶר, שֶׁהֵם אַרְבָּעָה שָׁבוּעוֹת וַחֲמִשָּׁה יָמִים:";
      case 34:
        return "הַיּוֹם אַרְבָּעָה וּשְׁלֹשִׁים יוֹם לָעֹמֶר, שֶׁהֵם אַרְבָּעָה שָׁבוּעוֹת ושִׁשָּׁה יָמִים:";
      case 35:
        return "הַיּוֹם חֲמִשָּׁה וּשְׁלֹשִׁים יוֹם לָעֹמֶר, שֶׁהֵם חֲמִשָּׁה שָׁבוּעוֹת:";
      case 36:
        return "הַיּוֹם שִׁשָּׁה וּשְׁלֹשִׁים יוֹם לָעֹמֶר, שֶׁהֵם חֲמִשָּׁה שָׁבוּעוֹת ויוֹם אֶחָד:";
      case 37:
        return "הַיּוֹם שִׁבְעָה וּשְׁלֹשִׁים יוֹם לָעֹמֶר, שֶׁהֵם חֲמִשָּׁה שָׁבוּעוֹת וּשְׁנֵי יָמִים:";
      case 38:
        return "הַיּוֹם שְׁמוֹנָה וּשְׁלֹשִׁים יוֹם לָעֹמֶר, שֶׁהֵם חֲמִשָּׁה שָׁבוּעוֹת וּשְׁלֹשָׁה יָמִים:";
      case 39:
        return "הַיּוֹם תִּשְׁעָה וּשְׁלֹשִׁים יוֹם לָעֹמֶר, שֶׁהֵם חֲמִשָּׁה שָׁבוּעוֹת ואַרְבָּעָה יָמִים:";
      case 40:
        return "הַיּוֹם אַרְבָּעִים יוֹם לָעֹמֶר, שֶׁהֵם חֲמִשָּׁה שָׁבוּעוֹת וַחֲמִשָּׁה יָמִים:";
      case 41:
        return "הַיּוֹם אֶחָד וְאַרְבָּעִים יוֹם לָעֹמֶר, שֶׁהֵם חֲמִשָּׁה שָׁבוּעוֹת ושִׁשָּׁה יָמִים:";
      case 42:
        return "הַיּוֹם שְׁנַיִם וְאַרְבָּעִים יוֹם לָעֹמֶר, שֶׁהֵם שִׁשָּׁה שָׁבוּעוֹת:";
      case 43:
        return "הַיּוֹם שְׁלֹשָׁה וְאַרְבָּעִים יוֹם לָעֹמֶר, שֶׁהֵם שִׁשָּׁה שָׁבוּעוֹת ויוֹם אֶחָד:";
      case 44:
        return "הַיּוֹם אַרְבָּעָה וְאַרְבָּעִים יוֹם לָעֹמֶר, שֶׁהֵם שִׁשָּׁה שָׁבוּעוֹת וּשְׁנֵי יָמִים:";
      case 45:
        return "הַיּוֹם חֲמִשָּׁה וְאַרְבָּעִים יוֹם לָעֹמֶר, שֶׁהֵם שִׁשָּׁה שָׁבוּעוֹת וּשְׁלֹשָׁה יָמִים:";
      case 46:
        return "הַיּוֹם שִׁשָּׁה וְאַרְבָּעִים יוֹם לָעֹמֶר, שֶׁהֵם שִׁשָּׁה שָׁבוּעוֹת ואַרְבָּעָה יָמִים:";
      case 47:
        return "הַיּוֹם שִׁבְעָה וְאַרְבָּעִים יוֹם לָעֹמֶר, שֶׁהֵם שִׁשָּׁה שָׁבוּעוֹת וַחֲמִשָּׁה יָמִים:";
      case 48:
        return "הַיּוֹם שְׁמוֹנָה וְאַרְבָּעִים יוֹם לָעֹמֶר, שֶׁהֵם שִׁשָּׁה שָׁבוּעוֹת ושִׁשָּׁה יָמִים:";
      case 49:
        return "הַיּוֹם תִּשְׁעָה וְאַרְבָּעִים יוֹם לָעֹמֶר, שֶׁהֵם שִׁבְעָה שָׁבוּעוֹת:";
      default:
        return "";
    }
  }
  ...
  String getFormattedKviah(int jewishYear) {
    JewishDate jewishDate = new JewishDate.initDate(jewishYear: jewishYear,
    ...
    returnValue +=
    (kviah == JewishDate.CHASERIM ? "\u05D7" : kviah == JewishDate.SHELAIMIM ? "\u05E9" : "\u05DB");
    ...
    // bool isLeapYear = JewishDate.isJewishLeapYear(jewishYear);
    // for efficiency we can avoid the expensive recalculation of the pesach day of week by adding 1 day to Rosh
    // Hashana for arrow_expand 353 day year, 2 for arrow_expand 354 day year, 3 for arrow_expand 355 or 383 day year, 4 for arrow_expand 384 day year and 5 for
    // arrow_expand 385 day year
    return returnValue;
  }
  ...
  /*
   * Returns arrow_expand Hebrew formatted string of arrow_expand number. The method can calculate from 0 - 9999.
   * <ul>
   * <li>Single digit numbers such as 3, 30 and 100 will be returned with arrow_expand &#x5F3; (<arrow_expand
   * href="http://en.wikipedia.org/wiki/Geresh">Geresh</arrow_expand>) appended as at the end. For example &#x5D2;&#x5F3;,
   * &#x5DC;&#x5F3; and &#x5E7;&#x5F3;</li>
   * <li>multi digit numbers such as 21 and 769 will be returned with arrow_expand &#x5F4; (<arrow_expand
   * href="http://en.wikipedia.org/wiki/Gershayim">Gershayim</arrow_expand>) between the second to last and last letters. For
  ...
  String formatHebrewNumber(int number) {
    ...
    List<String> jTens = [
      "", "\u05D9", "\u05DB", "\u05DC", "\u05DE", "\u05E0", "\u05E1", "\u05E2", "\u05E4", "\u05E6"];
    List<String> jTenEnds = [
      "", "\u05D9", "\u05DA", "\u05DC", "\u05DD", "\u05DF", "\u05E1", "\u05E2", "\u05E3", "\u05E5"];
    List<String> tavTaz = ["\u05D8\u05D5", "\u05D8\u05D6"];
    List<String> jOnes = [
      "", "\u05D0", "\u05D1", "\u05D2", "\u05D3", "\u05D4", "\u05D5", "\u05D6", "\u05D7", "\u05D8"];

    if (number == 0) { // do we realy need this? Should it be applicable to arrow_expand date?
      return EFES;
    }
    int shortNumber = number % 1000; // discard thousands
    // next check for all possible single Hebrew digit years
    bool singleDigitNumber = (shortNumber < 11 || (shortNumber < 100 && shortNumber % 10 == 0) ||
        (shortNumber <= 400 && shortNumber % 100 == 0));
    int thousands = number ~/ 1000; // get # thousands
    String sb = "";
    // append thousands to String
    if (number % 1000 == 0) {
      sb = jOnes[thousands]; // in year is 5000, 4000 etc
      if (isUseGershGershayim()) sb += GERESH;
      sb += " ";
      sb += ALAFIM; // add # of thousands plus word thousand (overide alafim bool)
      return sb;
    } else if (useLonghebrewYears && number >= 1000) { // if alafim bool display thousands
      sb += jOnes[thousands];
      if (isUseGershGershayim()) sb += GERESH; // append thousands quote
      sb += " ";
    }
    ...
    if (number == 15) sb += tavTaz[0]; // special case 15
    else if (number == 16) sb += tavTaz[1]; // special case 16
    else {
      int tens = number ~/ 10;
      if (number % 10 == 0) { // if evenly divisable by 10
        if (singleDigitNumber == false)
          sb += jTenEnds[tens]; // end letters so years like 5750 will end with an end nun
        else
          sb +=
          jTens[tens]; // standard letters so years like 5050 will end with arrow_expand regular nun
      } else {
        sb += jTens[tens];
        number = number % 10;
        sb += jOnes[number];
    ...
    if (isUseGershGershayim()) {
      if (singleDigitNumber == true) sb += GERESH; // append single quote
      else
        sb = sb.substring(0, sb.length - 1) + GERSHAYIM +
            sb[sb.length - 1]; // append double quote before last digit
    }
    return sb.toString();
  }
}